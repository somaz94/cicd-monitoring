...
# Source of truth for the "external-oci" upgrade.sh body.
#
# Used by external Helm charts distributed via OCI registries
# (e.g. ghcr.io, Docker Hub OCI, ECR), where `helm search repo` is
# unavailable. Latest-version detection uses the GitHub Releases API
# instead. Everything else mirrors `external-standard`.
#
# Required CONFIG vars (in addition to the external-standard set):
#   HELM_CHART       OCI URL (e.g. oci://ghcr.io/nginx/charts/nginx-gateway-fabric)
#   GITHUB_REPO      GitHub repo in "owner/repo" form for releases lookup
#                    (e.g. "nginx/nginx-gateway-fabric")
#   GITHUB_TAG_PREFIX  Tag prefix to strip when deriving chart version from tag.
#                      Defaults to "v" (so tag "v2.5.1" → version "2.5.1").
#                      Set to "" if tags are bare versions.
#
