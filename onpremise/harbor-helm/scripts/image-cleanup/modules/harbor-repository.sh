...
get_repositories() {
    ...
    debug_print "Repository list URL: $url"
    ...
    if echo "$response" | jq -e '.errors' >/dev/null 2>&1; then
        ...
        echo -e "${RED}Server error: $error_msg${NC}"
        return 1
    fi

    # Extract repository names and strip project prefix
...
get_repository_info() {
    ...
    echo -e "${YELLOW}Fetching repository info: $url${NC}"
...
# Extract artifact count for a repository
get_artifact_count() {
    local repo=$1
    local repo_info=$2
...
# Direct repository info lookup
get_direct_repository_info() {
    local repo=$1
    echo -e "${YELLOW}Direct repository info lookup: $repo${NC}"
